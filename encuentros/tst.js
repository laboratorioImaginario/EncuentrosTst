...
function setup(){
   ...
   var col0101 = lerpColor(color(col00),color("#fff9ea"),0.4);
   col111 = col00;
   ...
   scl = int(rand_between(300,450));

   lerpCol = 60;
   intensidad0 = rand_between(1,2.5);
   intensidad1 = intensidad0/rand_between(1,2);
   intensidad00 = rand_between(1.25,2.25);
   addCol = int(rand_between(6,8));
   val00 = int(rand_between(15,18));
   intensidad3 = int(rand_between(1,3));
   intensidad4 = rand_between(0.25,1);
   value = rand_between(0,0.00005);
 //value = 0;
 ...
 console.log(scl);
 console.log("intensidad4: " + intensidad4);
 ...
 if(gridPts.nombre == "reticula"){
    ...
    var tamanosIguales = rand_between(12,18);
    console.log("reticula");  
    console.log("orden: " + _orden)  
    console.log("tamanoElipse: " + _tamanoElipse)  

    var val0111 = int(random(3,5));
    var val0222 = int(random(3,5));
    ...
    for(var i = numb/2; i<dim; i+=numb){
        ...
        for(var j = numb0/2; j<dim; j+=numb0){
            ...
            if(_orden == "noOrdenado"){
                gridptx = append(gridptx,i+rand_between(-numb/4,numb/4));
            ...
            if(_tamanoElipse == "diferentes"){
                gridptsz = append(gridptsz,rand_between(10,20));
            }
        }
    }
}
...
if(gridPts.nombre == "aleatorio"){
    var total = int(rand_between(50,200));
    var _tamanoElipse = gridPts.tamanoElipse[int(rand_between(0,gridPts.tamanoElipse.length))];
...
for (let i = 0; i < circles.length; i++) {
    ...
    if(_tamanoElipse == "diferentes"){
        gridptsz = append(gridptsz,rand_between(10,20));  //15-20
    }
}
}
...
if(flow.nombre == "poligono"){
    ...
    _rotacion = flow.rotacion[int(rand_between(0,flow.rotacion.length))];
    ...
    if(_rotacion == "iguales"){
      rotGeoFlow = random(TWO_PI);
  } 
  ...
  for(var ii = 0; ii<TWO_PI; ii+=TAU/geometria0){
    ...
    if(_rotacion == "diferentes"){
        rotGeoFlow = rotGeoFlow+random(TWO_PI);
    } 
    ...
    reduccionGrosorIgual0 = rand_between(0.025,0.1); 
        ...
        var tamCenters;
        if(geometria0 == 2){
            tamCenters = dim/rand_between(3,4);
        }
        if(geometria0 == 3){
            var rand01 = rand_between(2.4,3.8);
            tamCenters = dim/rand01;
        }
        if(geometria0 == 4){
            var rand01 = rand_between(2.3,3.5);
            tamCenters = dim/rand01;
        }
        ...
        if(geometria0 == 2){
            tamanoGeo = numb2/rand_between(1,1.1); //1
        }
        if(geometria0 == 3){
            tamanoGeo = numb2/rand_between(1.1,1.3); //1.1
        }
        if(geometria0 == 4){
            tamanoGeo = numb2/rand_between(1.3,1.4);;  //1.15
        }
    ...
    console.log("rotacion: " + _rotacion);
    ...
    if(geometria == 3){
        tamanoGeoFinal = tamanoGeo*rand_between(1,1.2);
    }
    if(geometria == 4){
    ...
    if(geometria0 == 2){
       numb = tamanoGeoFinal/22;
   }else{
    numb = tamanoGeoFinal/18;
}
        ...
        for(var i = 0; i<xpos1.length-1; i++){
            ...
            if(retic == 2){
               if(sz2[j] >= tamanoGeoFinal-10){
                ...
                steps = map_range(j,0,sz2.length,tamanoGeoFinal/6,0); 
            }
        }
...
for(var jj = 0; jj<steps; jj++){
    ...
    if(_esquemaColor == "color0"){
       ...
       if(trazo == "continuo"){
            florGrosorCombinado = append(florGrosorCombinado,rand_between(6,7));  //13

        }

        mixColVal = rand_between(0.4,0.6);
        reduccionGrosorCombinado = append(reduccionGrosorCombinado,rand_between(0.04,0.08));
        mixCol = append(mixCol,rand_between(0.15,0.7));
...
var g00 = green(col00);
var b00 = blue(col00);
var col001 = lerpColor(color(col00),color(0),0.3);
pg.fill(col00);
    //pg.noStroke();
...
for(var ii = numb11/2; ii<dim; ii+=numb11){
    ...
    for(var mm = 0; mm<dim; mm+=numb00){
        ...
        var randx = rand_between(-dim/5,dim/5);
        addColl0 = int(rand_between(3,6));
...
if(retic == 1){
        ...
        if(retic == 2){
        tamanoGeo = numb00/rand_between(0.7,1.1); //1
    }
    if(retic == 3){
            tamanoGeo = numb00/rand_between(0.6,0.9); //1
        }
    ...
    if(geometria == 3){
        tamanoGeoFinal = tamanoGeo*rand_between(0.9,1.1);
    }
    if(geometria == 4){
        tamanoGeoFinal = tamanoGeo*rand_between(0.7,0.9);
    }

    if(retic == 1){
    numb = tamanoGeoFinal/19;  //22
}
if(retic == 2){
    numb = tamanoGeoFinal/14;  //22
}
if(retic == 3){
    numb = tamanoGeoFinal/18;  //22
}
for(var j = tamanoGeoFinal; j>5; j-=numb){
    sz2 = append(sz2,j);
}
...
for(var j = 0; j<gridptx.length; j++){
    var sz = gridptsz[j]/1.25;

    var col012 = lerpColor(color(col111),color(250),0.1);
...
function draw() {
    ...
    for(var i = 0; i<flowptx.length; i++){
      ...
      for(var j = 0; j<gridptx.length; j++){
          ...
          let d = sqrt((xdif * xdif) + (ydif * ydif))*intensidad4;
          var d0 = dist(flowptx[i],flowptx[i],gridptx[i],gridpty[i]);
          angle +=  abs(gridptsz[j]/2  / d*intensidad1);
      }
      ...
      if(flowVal == "flow0"){
        sclVal = scl;
        angle0 = abs(noise(flowptx[i]/sclVal,flowpty[i]/sclVal,i*value)*TWO_PI*intensidad0);
    }
    if(flowVal == "flow1"){
        let ns = 0.0015;
        let n = 10;
        angle0 = int(noise(flowptx[i] * ns, flowpty[i] * ns,i*0.00001) * 22) * PI / 4;
        sclVal = scl*2;
...
if(trazo == "manchas"){
    if(grosor == "iguales"){
        if(frameCount <= 3){
          ...
          strokeWeight(rand_between(flowGrosorIgual*0.5,flowGrosorIgual*1.3));
      }
      
  }
  if(grosor == "diferentes"){
    strokeWeight(rand_between(0,florGrosorCombinado[i]*1.5));  
}
if(grosor == "igualcu"){
 if(frameCount <= 3){
   strokeWeight(flowGrosorIgual00[i]);
}else{
    strokeWeight(rand_between(flowGrosorIgual00[i]*0.5,flowGrosorIgual00[i]*1.3));
}
}
}
noFill();
...
if(_esquemaColor == "color2"){
    var n = map_range(frameCount,0,lerpCol,0,1);
    ...
    if(_esquemaColor == "color3"){
        var n = map_range(frameCount,0,lerpCol,0,1);  //mas valor mas tarda
